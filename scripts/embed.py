...
def build_embedding_text(chunk: dict) -> str:
    ...
    CHANGED: Use 'document_title' instead of 'source'
    Keep 'page' for context
    """
    ...
    header_parts = []
    if doc_title:
        header_parts.append(f"Source: {doc_title}")
...
def main() -> None:
    ...
    # Optional: Show sample chunk structure for debugging
    if chunks:
        print(f"[embed] Sample chunk fields: {list(chunks[0].keys())}")
        print(f"[embed] Sample doc_title: {chunks[0].get('document_title', 'N/A')}")
        print(f"[embed] Sample page: {chunks[0].get('page', 'N/A')}")
        print()
